...
class WordFrequency extends Component
{
	 ...
	 * with array_walk_recursive to retrieve all values present.
	 */
	public $blackListRegularExpression = [];
	 ...
	 */
	public $blackListRegularExpressionFile = [];
	...
	*/
	...
	public $whiteListRegularExpression = [];
	public $whiteListRegularExpressionFile = [];
	...
	public $substitutionListFile = [];
	public $substitutionListRegularExpression = [];
	public $substitutionListRegularExpressionFile = [];
	...
	public function addSource($newSource) {
		if (is_string($newSource) or is_array($newSource)) {
		    ...
		    Yii::warning(Yii::t('yii','Invalid source type in "{method}". Active Record Model and DbCriteria objects expected.', ['{method}'=>__METHOD__]));
		}
		return $this;
	}
	 ...
	 * @return $this WordFrequency object which allows method chaining
	 */
	...
	*/
	...
	protected function accumulateFromStringFile() {
		foreach ($this->sourceFileList as $v) {
			if (file_exists($v)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Token Source File "{file}" not found in {method}.', array('{method}'=>__METHOD__, '{file}'=>$v)));
			}
		}
	}
			...
			foreach ($this->sourceList as $v) {
				if (is_string($v)) {
					...
					if (is_array($v[0])) {
						...
						throw new InvalidConfigException(Yii::t('yii', 'Invalid string source in class {class}.', array('{class}'=>__CLASS__)));
					}
				}
			}
		}
	...
	protected function blackListFileFilter() {
		...
		foreach ($this->blackListFile as $v) {
			...
			if (file_exists($filePathName)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Blacklist File "{file}" not found in {method}.',
	...
	protected function blackListRegularExpressionFileFilter() {
		...
		foreach ($this->blackListRegularExpressionFile as $v) {
			...
			if (file_exists($filePathName)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Blacklist Regular Expression File "{file}" not found in {method}.',
											array('{method}'=>__METHOD__, '{file}'=>$filePathName)));
			}
		}
		$this->blackListRegularExpressionRemovalUtility($compositeBlackList);
	...
	protected function whiteListFileFilter() {
		...
		foreach ($this->whiteListFile as $v) {
			...
			if (file_exists($filePathName)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Whitelist File "{file}" not found in {method}.',
											array('{method}'=>__METHOD__, '{file}'=>$filePathName)));
			}
		}
		$this->whiteListRemovalUtility($compositeWhiteList);
	...
	protected function whiteListRegularExpressionFileFilter() {
		...
		foreach ($this->whiteListRegularExpressionFile as $v) {
			...
			if (file_exists($filePathName)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Whitelist Regular Expression File "{file}" not found in {method}.',
											array('{method}'=>__METHOD__, '{file}'=>$filePathName)));
			}
		}
		$this->whiteListRegularExpressionRemovalUtility($compositeWhiteList, true);
	...
	protected function whiteListRegularExpressionRemovalUtility($regularExpressionList) {
		...
		$partialList = []; //initialize, list of found items
		foreach ($regularExpressionList as $v) {
	...
	protected function substitutionListFileFilter() {
		...
		foreach ($this->substitutionListFile as $v) {
			...
			if (file_exists($filePathName)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Substitutionlist File "{file}" not found in {method}.',
											array('{method}'=>__METHOD__, '{file}'=>$filePathName)));
			}
		}
	...
	protected function substitutionListRegularExpressionFileFilter() {
		...
		foreach ($this->substitutionListRegularExpressionFile as $v) {
			...
			if (file_exists($filePathName)) {
				...
				throw new InvalidConfigException(Yii::t('yii', 'Substitutionlist Regular Expression File "{file}" not found in {method}.',
											array('{method}'=>__METHOD__, '{file}'=>$filePathName)));
			}
		}
	...
	protected function issueUsageWarnings() {
		if (!$this->accumulateVisited) {
		    Yii::warning(Yii::t('yii','Sources have not been accumulated in "{class}".', array('{class}'=>__CLASS__)));
		}

		if (count($this->sourceList) == 0) {
		    Yii::warning(Yii::t('yii','No sources defined in "{class}".', array('{class}'=>__CLASS__)));
		}

		if (count($this->tokenList) == 0) {
		    Yii::warning(Yii::t('yii','Sources have produced no tokens "{class}".', array('{class}'=>__CLASS__)));
		}
		...
		if (($countCheck) and (!$this->blackListVisited)) {
		    Yii::warning(Yii::t('yii','Blacklist defined but not used in "{class}".', array('{class}'=>__CLASS__)));
		}
		...
		if (($countCheck) and (!$this->whiteListVisited)) {
		    Yii::warning(Yii::t('yii','Whitelist defined but not used in "{class}".', array('{class}'=>__CLASS__)));
		}
		...
		if (($countCheck) and (!$this->substitutionListVisited)) {
		    Yii::warning(Yii::t('yii','Substitution List defined but not used in "{class}".', array('{class}'=>__CLASS__)));
		}
	}
